...
current_date = dt.datetime.now().date() - dt.timedelta(days=2)
...
if current_date.weekday() != 5 and current_date.weekday() != 6:
    try:
        ...
        minus_day_data = float(data["Time Series (Daily)"][str(minus_day)]["4. close"])
        print('=minus day data: ', minus_day_data)
        print(f'date {minus_day} stocks close price: ', minus_day_data)
...
#TODO 4. - Work out the percentage difference in price between closing price yesterday and closing price the day before yesterday.
    ...
    pointer = "UP"
    if minus_day_data < minus_2days_data:
        pointer = "DOWN"
...
#TODO 9. - Send each article as a separate message via Twilio. 
    ...
    password = "zgxqwxoxwveerkjq"
    
    message = f'{STOCK_NAME}: {pointer} {perc_print}%'  
    message_part1 = headlines[0].encode('utf-8')
    message_part2 = description[0].encode('utf-8')
    message_article = f"{message_part1} +\n+ {message_part2}"

    if len(headlines) != 0:   
        connection =  smtplib.SMTP("smtp.gmail.com")
        connection.starttls()
        connection.login(user=my_email, password=password)
        connection.sendmail(from_addr=my_email, to_addrs="o.korotushko@yahoo.com", msg=f"Subject:Stock_Trade_Info\n\n{message}")
        
        connection.sendmail(from_addr=my_email, to_addrs="o.korotushko@yahoo.com", msg=f"Subject:Stock_Trade_Article\n\n{message_article}")
